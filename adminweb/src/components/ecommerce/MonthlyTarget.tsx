...
export default function MonthlyTarget() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setThisMonthCount(currentCount);
        setLastMonthCount(prevCount);

        if (prevCount === 0) {
          setPercentChange(currentCount > 0 ? 100 : 0);
        } else {
  ...
  const series = [Math.min(100, Math.abs(percentChange))];
  const options: ApexOptions = {
    colors: [isIncrease ? "#039855" : "#D92D20"],
    chart: {
      fontFamily:
      ...
      radialBar: {
        ...
        dataLabels: {
          ...
          value: {
            ...
            color: "#1D2939",
            formatter: function () {
              return (isIncrease && percentChange > 0 ? "+" : "") + percentChange + "%";
            },
          },
        },
      },
      ...
      <div className="px-5 pt-5 bg-white shadow-default rounded-2xl pb-11 dark:bg-gray-900 sm:px-6 sm:pt-6">
        <div className="flex justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Biến động phiếu cứu hộ
            </h3>
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              So sánh số lượng phiếu tháng này với tháng trước
            </p>
          </div>
        ...
        <div className="relative ">
          ...
          <span className={`absolute left-1/2 top-full -translate-x-1/2 -translate-y-[95%] rounded-full px-3 py-1 text-xs font-medium ${
            ...
            {isIncrease && percentChange > 0 ? "+" : ""}{percentChange}%
          </span>
        </div>
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          {isIncrease 
            ? "Số lượng phiếu cứu hộ tăng so với tháng trước." 
            : "Số lượng phiếu cứu hộ giảm so với tháng trước."}
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-5 px-6 py-3.5 sm:gap-8 sm:py-5">
        ...
        <div>
          ...
          <p className={`flex items-center justify-center gap-1 text-base font-semibold sm:text-lg ${
            isIncrease ? "text-success-600" : "text-error-600"
          }`}>
